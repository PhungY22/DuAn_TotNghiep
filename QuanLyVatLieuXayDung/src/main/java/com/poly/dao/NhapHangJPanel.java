...
public class NhapHangJPanel extends javax.swing.JPanel {
    ...
    Map<String, String> cungCapMap = new HashMap<>();

    Map<String, String> nhanVienMap = new HashMap<>();

    Map<String, BigDecimal> tinhTongMap = new HashedMap<>();
...
// Format the date as "dd/MM/yyyy HH:mm:ss"
    ...
    public void fillTable() {
        ...
        // Format the ngayXuat date as "dd/MM/yyyy HH:mm:ss"
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        ...
        try {
            ...
            Map<String, BigDecimal> tinhTongMap = new HashMap<>();

            List<PhieuNhapHang> list = nhapHangDao.selectAll();

            for (PhieuNhapHang phieuNH : list) {
                String maNhaCC = phieuNH.getMaNhaCungCap();
                String tenNhaCC = cungCapMap.get(maNhaCC);

                List<ChiTietPhieuNhap> chiTietList = chiTietPhieuNhapHangDAO.findMaPhieuNhapByPhieuNH(phieuNH.getMaPhieuNhapHang());

                BigDecimal tongTien = BigDecimal.ZERO;

                for (ChiTietPhieuNhap ctpn : chiTietList) {
                    tongTien = tongTien.add((ctpn != null && ctpn.getTongTien() != null) ? ctpn.getTongTien() : BigDecimal.ZERO);
                }

                tinhTongMap.put(phieuNH.getMaPhieuNhapHang(), tongTien);

                Object[] row = {
                    phieuNH.getMaNhaCungCap(),
                    tenNhaCC,
                    phieuNH.getMaPhieuNhapHang(),
                    phieuNH.getMaNhanVien(),
                    phieuNH.getVatlieu(),
                    phieuNH.getHinhThucThanhToan(),
                    phieuNH.getSoPhieuNhap(),
                    tongTien
                };
                model.addRow(row);
            }
    ...
    private void setForm(PhieuNhapHang obj) {
        txtMaCungCap.setText(obj.getMaNhaCungCap());

        // Format the date (ngayNhap) as "dd/MM/yyyy"
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");

        String ngayNhapString = dateFormat.format(obj.getNgayNhap());

        System.out.println(" ngay nhap" + ngayNhapString);

        txtNgayNhap.setText(ngayNhapString);

        String tenKhachHang = cungCapMap.get(obj.getMaNhaCungCap());
        // Set the customer name to txtTenKhachHang

        cbNhaCungCap.setSelectedItem(obj.getMaNhaCungCap());

        txtTenNhaCungCap.setText(tenKhachHang);

        txtMaPhieuNhap.setText(obj.getMaPhieuNhapHang());
...
//        String tenVoucher = voucherMap.get(obj.getMaVoucher());
        String SoPhieuNhap = obj.getSoPhieuNhap();
        txtSoPhieuNhap.setText(SoPhieuNhap);

        cbVatLieu.setSelectedItem(obj.getVatlieu());

        int selectedRow = tblDanhSachNH.getSelectedRow();

        if (selectedRow != -1) { // Check if a row is selected
            DefaultTableModel model = (DefaultTableModel) tblDanhSachNH.getModel();

            String tinhtong = model.getValueAt(selectedRow, 7).toString();

            txtTongTien.setText(tinhtong);
        }
    ...
    public void getNhaCungCap() {
        try {
            List<NhaCungCap> nhaCungCaps = cungCapDao.selectAll();

            // Clear existing items in the JComboBox
            cbNhaCungCap.removeAllItems();

            // Add NhaCungCap items to the JComboBox
            for (NhaCungCap nhaCungCap : nhaCungCaps) {

                cbNhaCungCap.addItem(nhaCungCap.getMaNhaCungCap());

                cungCapMap.put(nhaCungCap.getMaNhaCungCap(), nhaCungCap.getTenNhaCungCap());
    ...
    public void getVatLieu() {
        try {
            List<PhieuNhapHang> phieuNhapHangs = nhapHangDao.selectAll();
            ...
            for (PhieuNhapHang phieunhaphang : phieuNhapHangs) {
                cbVatLieu.addItem(phieunhaphang.getVatlieu());

                System.out.println(" " + phieunhaphang.getMaNhaCungCap());
                System.out.println(" " + phieunhaphang.getVatlieu());
    ...
    private void edit() {
        if (this.row >= 0) {
            String id = (String) tblDanhSachNH.getValueAt(this.row, 0);
            PhieuNhapHang pnh = nhapHangDao.selectById(id);
            this.setForm(pnh);
        }
    }
    ...
    private void next() {
        int rowCount = tblDanhSachNH.getRowCount();
    ...
    private void selectAndScrollToVisible() {
        tblDanhSachNH.setRowSelectionInterval(this.row, this.row);
        tblDanhSachNH.scrollRectToVisible(tblDanhSachNH.getCellRect(this.row, 0, true));
        this.edit();
    }
    ...
    void ASC() {
        ...
        tblDanhSachNH.setRowSorter(sorter);
...
//
    ...
    void search(String str) {
        ...
        tblDanhSachNH.setRowSorter(trs);
                                ...
                                .addGroup(jPanel6Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(jPanel6Layout.createSequentialGroup()
                                        ...
                                        .addGroup(jPanel6Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                            ...
                                            .addComponent(txtMaPhieuNhap, javax.swing.GroupLayout.PREFERRED_SIZE, 179, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                    .addGroup(jPanel6Layout.createSequentialGroup()
                                        .addGap(16, 16, 16)
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    ...
                    .addComponent(txtNguoiGiao, javax.swing.GroupLayout.PREFERRED_SIZE, 32, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
